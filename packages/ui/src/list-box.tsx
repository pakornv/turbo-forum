...
const listBoxItemStyles = tv({
  ...
  variants: {
    ...
    isHovered: {
      true: "bg-accent text-accent-fg [&_.text-muted-fg]:text-accent-fg/80 [&:hover_[slot=description]]:text-accent-fg/70 [&:hover_[slot=label]]:text-accent-fg",
    },
    isFocused: {
  ...
  return (
    <ListBoxPrimitive
      className={composeTailwindRenderProps(
        className,
        "grid grid-cols-[auto_1fr] overflow-auto py-1 outline-hidden *:[[role='group']+[role=group]]:mt-4 *:[[role='group']+[role=separator]]:mt-1",
      )}
      {...props}
    />
  );
